...
if myStep==0;
	...
	%fprintf(fid,'\usepackage{pgfpages}\n');
	%fprintf(fid,['\pgfpackagesuselayout{resize to}[a4paper,' ...
        %             'landscape,border shrink=5mm]\n']);
        %
        fprintf(fid,'\n');
...
if myStep==5;
    ...
    system(['latex ' fileTex]);
    system(['dvips -t unknown ' fileTex]);
    system(['ps2pdf ' fileTex '.ps']);
    cd(dirOrig);
end;
