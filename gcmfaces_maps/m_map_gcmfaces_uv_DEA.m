...
function []=m_map_gcmfaces_uv_DEA(fldU,fldV,varargin);
...
m_proj('mercator','lon',[-180 180],'lat',[-89 89]);
%m_proj('mollweide','lon',[-100 20],'lat',[-20 70]);
...
m_grid;
hold on; if isempty(scaleFac); quiver(x,y,u,v,'w'); else; quiver(x,y,u,v,scaleFac,'w'); end;
end;%if choicePlot==0|choicePlot==1; 

if choicePlot==0; subplot(2,1,1); end;

if choicePlot==0|choicePlot==1; 
m_proj('Mercator','lat',[-70 70],'lon',[-180 180]);
