function []=m_map_gcmfaces_DAmovie2(fld,varargin);
...
if (choicePlot~=0&choicePlot~=1&choicePlot~=2&choicePlot~=3);
    do_my_plot(fld,param,choicePlot,myShading,cLong);
...
if plotCBAR==2&strcmp(myPlot,'pcolor')&doCbar;
    ...
    elseif choicePlot~=2&choicePlot~=3;
%         set(cbar,'Position',[0.92 0.23 0.02 0.55]);
        pos=get(cbar,'Position'); pos(1)=pos(1)-0.1; set(cbar,'Position',pos);
    end;
end;
...
function []=do_my_plot(fld,param,proj,shad,cLong);
...
m_grid_opt=['''XaxisLocation'',xloc,''YaxisLocation'',yloc,''xtick'',xtic,''ytick'',ytic,''linestyle'',''none'''];
