...
elseif userStep==3;%computation
    ...
    %
    % If we're past the beginning of the averaging interval, load
    % the files that we computed at the last time step.
    ...
    for jj=1:length(listDiags)/2;
        ...
        % changed to <= by DEA for average intervals that don't
        % span the whole integration (otherwise _s and _2s
 ...
 %load last cumsum
  ...
  if ifile0>0;
    ...
    for ii=1:length(alldiag.listDiags);
         ...
         % set alldiag to be the difference between the first and
         % last cumsum, for all fields that aren't just lists
  ...
  %
% Take the average by dividing the difference between cumsums at
% the beginning and end of the average intervals by the number of
% years. 
...
%===== start with state variables
  ...
  if ( sum(strcmp(choicePlot,'all'))|sum(strcmp(choicePlot,'subsrfc')) )&(~doOmit3Dfields);
    ...
    if length(mygrid.DRF) ==50
        kkList=[1 11 20 28 37 44];
    else
        kkList=[1 5 10 15 18 20];
    end
