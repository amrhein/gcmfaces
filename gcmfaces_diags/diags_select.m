...
% listDiags is created by diags_set_*
...
% Added by DEA. Some variables are not declared
%vnd = logical(zeros(1,length(listDiags)));
%for ii = 1:length(listDiags)
%    if ~exist(listDiags{ii},'var')
%        vnd(ii) = 1;
%    end
%end
%listDiags(vnd) = [];
...
for iChunk=listChunk;
    ...
    for ii=i0+1:i1;
        ...
        % DEA change to 360
        onediag.listTimes=myparms.yearFirst(1)+tt*myparms.timeStep/86400/360;
