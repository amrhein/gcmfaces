...
fileName0=fullfile(dirModel,'diags',filesep,'BUDG',filesep,fileList(ii).name(1:end-5));
meta0=rdmds_meta(fileName0);
fileName1=fullfile(dirModel,'diags',filesep,'BUDG',filesep,fileList(ii+1).name(1:end-5));
...
%(2D diagnostics are expectedly vertically integrated by MITgcm)
  for jj=0:1;
    ...
    etanName=fullfile(dirModel,'diags',filesep,'BUDG',filesep,etanName);
