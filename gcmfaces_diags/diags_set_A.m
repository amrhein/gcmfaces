% DEA path to llc45_basin (mask for llc45)
addpath /nobackupp8/damrhein/MITgcm/mysetups/global_oce_llc45/analysis
% DEA set whether to use pre-defined y axes for plots
useYlims = 0;

if userStep==1;%diags to be computed
    listDiags='fldBAR fldTRANSPORTS gloOV gloOVbolus gloOVres gloMT_H gloMT_FW gloMT_SLT';
    listBasins=1;
    if min(mygrid.ioSize)==45;
        listBasins=[1:3];
        mskC=llc45_basin({'atlExt'});
        if isempty(mskC); listBasins=1; end;
    elseif min(mygrid.ioSize)==90;
        listBasins=[1:3];
        mskC=v4_basin({'atlExt'});
        if isempty(mskC); listBasins=1; end;
    else
        error('Not configured for this grid!')    
    end;
...
%if userStep==1;%diags to be computed
               %    listDiags='fldBAR fldTRANSPORTS gloOV gloOVbolus gloOVres gloMT_H gloMT_FW gloMT_SLT';
               %    listDiags='fldBAR fldTRANSPORTS';
               %    if sum([90 1170]/2~=mygrid.ioSize)==0;
               %      listDiags=[listDiags ' atlOV atlOVbolus atlOVres atlMT_H atlMT_FW atlMT_SLT'];
      %      listDiags=[listDiags ' pacindOV pacindOVbolus pacindOVres pacindMT_H pacindMT_FW pacindMT_SLT'];
      %    end;
...
elseif userStep==3;%computational part;
        ...
        for bb=listBasins;
            ...
            elseif bb==3;
                if min(mygrid.ioSize)==45;
                    mskC=llc45_basin({'pacExt','indExt'});
                    mskC=mk3D(mskC,fldU);
                elseif min(mygrid.ioSize)==90;
                    mskC=v4_basin({'pacExt','indExt'});
                    mskC=mk3D(mskC,fldU);
                end
            end;
...
elseif userStep==-1;%plotting
    ...
    % DEA
    axis tight
    set(get(cbar,'ylabel'),'string','Sverdrups')
    ylabel('Depth (m)')
    xlabel('Latitude')
    ...
    gloOV1000=squeeze(alldiag.gloOV(:,kk,:)); gloOV1000= ...
              runmean(gloOV1000,myNmean,2);
% modified by DEA, Sept 2015 to get these axes correct
    plot(TTa,gloOV1000(90+25,:),'LineWidth',2); hold on; plot(TTa,gloOV1000(90+35,:),'k','LineWidth',2);
    plot(TTa,gloOV1000(90+45,:),'r','LineWidth',2); plot(TTa,gloOV1000(90+55,:),'g','LineWidth',2);
    title0='annual global overturning at \approx 1000m depth (Sv)'; aa=axis; aa()
    legend('25N','35N','45N','55N'); title(title0);
    if useYlims, aa(3:4)=[0 20]; axis(aa); end
    grid on;
    if doAnomalies && useYlims; aa(3:4)=scaleAnom*[-1 1]; axis(aa); end;
    ...
    plot(TTa,atlOV1000(90+25,:),'LineWidth',2); hold on; plot(TTa,atlOV1000(90+35,:),'k','LineWidth',2);
    plot(TTa,atlOV1000(90+45,:),'r','LineWidth',2); plot(TTa,atlOV1000(90+55,:),'g','LineWidth',2);
    title0='annual atlantic overturning at \approx 1000m depth';
    legend('25N','35N','45N','55N'); title(title0);
    if useYlims, aa(3:4)=[0 20]; axis(aa); end
    grid on;
    if doAnomalies & useYlims; aa(3:4)=scaleAnom*[-1 1]*2; axis(aa); end;
    ...
    % DEA
    % The following two sections plot Hovmoller diagrams. They have
    % been changed to accomodate decadal output.

    %meridional heat transport
    %    fld=squeeze(alldiag.gloMT_FW(:,tt))';
    %    x=TT*ones(1,length(mygrid.LATS)); y=ones(nt,1)*mygrid.LATS';
    fld=squeeze(alldiag.gloMT_H(:,1:length(TTa)))';
    x=TTa*ones(1,length(mygrid.LATS)); y=ones(length(TTa),1)*mygrid.LATS';
    ...
    %meridional salt transport
    fld=squeeze(alldiag.gloMT_SLT(:,1:length(TTa)))';
    x=TTa*ones(1,length(mygrid.LATS)); y=ones(length(TTa),1)*mygrid.LATS';
    fld=runmean(fld,myNmean,1);
    %    cc=[[-250:50:-100] [-75 -50] [-35:10:35] [50 75] [100:50:250]]/100;
    if doAnomalies; cc=scaleAnom*[-1:0.1:1]*0.05; end;
    figureL; pcolor(x,y,fld); shading flat; axis([TTa(1) TTa(end) -90 90]);
    cc = round(linspace(min(caxis),max(caxis),20));
    gcmfaces_cmap_cbar(cc); 
    title('Meridional Salt Transport (in psu.Sv)');
    ...
    for iii=1:length(iiList);
        ...
        if useYlims
            disp_transport(trsp,TTa,txt,{'ylim',ylim},{'nmean',myNmean});
        else
            disp_transport(trsp,TTa,txt,{'nmean',myNmean});
        end
    end;
    ...
    if useYlims
        disp_transport(trsp,TTa,txt,{'ylim',ylim},{'nmean',myNmean},{'choicePlot',2});
    else
        disp_transport(trsp,TTa,txt,{'nmean', ...
                            myNmean},{'choicePlot',2});
    end
    ...
    for iii=1:length(iiList);
        ...
        if useYlims
            disp_transport(trsp,TTa,txt,{'ylim',ylim},{'nmean', ...
                                myNmean});
        else
            disp_transport(trsp,TTa,txt,{'nmean', myNmean});
        end
