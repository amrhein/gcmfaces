...
elseif userStep==-1;%plotting
    ...
    if min(mygrid.faces2gcmSize) == 45
        X=circshift(X,[-90 0]); X(X<0)=X(X<0)+360;
    else
...
%DEA replaced TTa with TTa
    plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2);
    aa=axis; axis([min(TTa) max(TTa) aa(3:4)]); grid on;
    ...
    figureL; set(gcf,'Renderer','zbuffer'); cc0=[-2:0.25:2];
    X=TTa*ones(1,length(mygrid.RC)); Y=ones(length(TTa),1)*(mygrid.RC'); X=X'; Y=Y';
    %T
    subplot(2,1,1);
    ...
    x=TTa*ones(1,length(mygrid.LATS)); y=ones(nta,1)*mygrid.LATS'; x=x'; y=y';
    %T
    subplot(2,1,1);
    ...
    pcolor(x,y,fld); shading interp; axis([TTa(1) TTa(end) -90 90]); cbar=gcmfaces_cmap_cbar(cc);
    ...
    pcolor(x,y,fld); shading interp; axis([TTa(1) TTa(end) -90 90]);
    ...
    plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 20]);
    ...
    plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 25]);
    ...
    plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 50]);
    ...
    plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 15]);
    ...
    plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 5]);
    ...
    plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 4]);
    ...
    if ~doAnomalies; %does not work for anomalies -> need to compute ratio in basic_diags_ecco
        ...
        plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 2]);
        ...
        plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 0.4]);
        ...
        plot(TTa,vec,'LineWidth',2); hold on; plot(TTa,runmean(vec,myNmean,2),'r','LineWidth',2); axis([min(TTa) max(TTa) 0 0.25]);
